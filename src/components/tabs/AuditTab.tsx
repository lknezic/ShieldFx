...
import { AlertTriangle, Ban, Eye, CheckCircle, MessageSquare, Mail, Plus, Send, FolderInput, RefreshCw, BellOff, BellRing, ShieldOff } from "lucide-react";
...
const ACTION_CONFIG: Record<ActionType, { icon: React.ReactNode; color: string; label: string }> = {
  ...
  FOLDER_MOVE: { icon: <FolderInput className="h-3.5 w-3.5" />, color: "text-blue-400", label: "Folder Changed" },
  RETRIGGER: { icon: <RefreshCw className="h-3.5 w-3.5" />, color: "text-orange-400", label: "Re-triggered" },
  TAG_SNOOZED: { icon: <BellOff className="h-3.5 w-3.5" />, color: "text-muted-foreground", label: "Tag Snoozed" },
  TAG_UNSNOOZED: { icon: <BellRing className="h-3.5 w-3.5" />, color: "text-primary", label: "Tag Unsnoozed" },
  AUTO_SUSPEND: { icon: <ShieldOff className="h-3.5 w-3.5" />, color: "text-red-400", label: "Auto-Suspended" },
};

function AuditEntryRow({ entry }: { entry: AuditEntry }) {
  const config = ACTION_CONFIG[entry.actionType] || ACTION_CONFIG.NOTE;
    ...
    <div className="flex gap-3 p-4">
      ...
      <div className="flex-1 min-w-0 pb-4">
        <div className="flex items-center gap-2 mb-1">
          ...
          {entry.violationRule && (
            <Badge variant="secondary" className="text-[10px] px-1.5">{entry.violationRule}</Badge>
          )}
          {entry.fromFolder && entry.toFolder && (
        ...
        <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
          ...
          {entry.emailSent && (
            <span className="flex items-center gap-1 text-primary">
              <Mail className="h-3 w-3" /> Sent to {entry.emailRecipient}
            </span>
          )}
        </div>
      </div>
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground">Complete history of actions on this account</p>
        </div>
        <Button variant="outline" size="sm" className="text-xs gap-1.5" onClick={() => setShowNoteForm(!showNoteForm)}>
      ...
      {showNoteForm && (
        <div className="border border-border rounded-lg p-4 bg-card">
          ...
          <textarea
            ...
            placeholder="e.g., Trader responded \u2014 claims using VPN. Escalating for review."
            rows={3}
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button size="sm" className="text-xs gap-1" onClick={handleAddNote}><Send className="h-3 w-3" /> Save Note</Button>
          </div>
        </div>
      )}
      ...
      {sortedAudit.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-12 text-center">
          ...
          <p className="text-xs text-muted-foreground max-w-sm">Actions will appear here as they are taken.</p>
        </div>
      ) : (
        <div className="border border-border rounded-lg overflow-hidden divide-y divide-border">
          {sortedAudit.map((entry) => <AuditEntryRow key={entry.id} entry={entry} />)}
        </div>
      )}
      ...
      {sortedAudit.length > 0 && (
        <div className="grid grid-cols-5 gap-2">
          {[
            ...
            { label: "Suspensions", count: sortedAudit.filter(e => e.actionType === "SUSPENSION" || e.actionType === "AUTO_SUSPEND").length, color: "text-destructive" },
            { label: "Moves", count: sortedAudit.filter(e => e.actionType === "FOLDER_MOVE").length, color: "text-blue-400" },
            { label: "Reviews", count: sortedAudit.filter(e => e.actionType === "REVIEW" || e.actionType === "CLEARED").length, color: "text-primary" },
